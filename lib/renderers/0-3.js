...
export default class Renderer {
  ...
  render() {
    this.sections.forEach((section, index) => {
      ...
      if (rendered) {
        rendered.__mobiledocIndex = index;
  ...
  renderListSection([type, tagName, listItems, attrs = []]) {
    ...
    listItems.forEach((li, index) => {
      const listElement = this.renderListItem(li);
      listElement.__mobiledocChildIndex = index;
      element.appendChild(listElement);
    });

    const attrsObj = reduceAttributes(attrs);
